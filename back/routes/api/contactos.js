...
router.get("/searchName/:nombre", async(req,res)=>{
    ...
    const contactos = await Contactos.findAll({where:{nombre:req.params.nombre},},{
...
router.post("/",middlewares.rol,async (req,res)=>{
   ...
   /*  var cabecera = req.headers["user-token"];
    ...
    const contactoEmail =  await Contactos.findOne({where:{email:req.body.email }});
    if(contactoEmail){
        
        
        res.json({error: " Ese contacto ya se encuentra registrado en la base de datos Intente ingresarlo con un email distinto"});
...
router.post("/porImportacion",middlewares.rol,async (req,res)=>{
    ...
    for (let index = 0; index < contactosBody.length; index++) {
        ...
        const contactoEmail =  await Contactos.findOne({where:{email:contactoActual.email }});
        if(!contactoEmail){
...
router.put("/",middlewares.rol,async (req,res)=>{
  ...
  const  contacto= await Contactos.update(obj,{ where:{ id:datoToUpdate} });
  ...
  const contactoCanalesDatos =  await ContactoCanales.findAll({where: {contactoId:datoToUpdate}});

  for (let index = 0; index < contactoCanalesDatos.length; index++) {
       contactoCanalesDatos[index].id;
       console.log( contactoCanalesDatos[index].Id, "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa");

        await ContactoCanales.update(datoCanal01,{where: {id:contactoCanalesDatos[0].Id}})
        await ContactoCanales.update(datoCanal02,{where: {id:contactoCanalesDatos[1].Id}})
