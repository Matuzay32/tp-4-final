...
let containerListaProvincias = document.getElementById(
	"containerListaProvincias"
);
...
let habilitarBotonesBorrarProvincia = () => {
	let botonesborrarProvincia =
		document.getElementsByClassName("btnborrarProvincia");
...
const agregarNuevaProvinciaAListaProvinciasCiudad = (
	idProvincia,
	nombreCiudad
) => {
...
btnAgregarProvincia.addEventListener("click", () => {
	agregarProvincia(
		crearProvinciasBaseDatos(
			document.getElementById("agregarProvinciaInput").value,
			document.getElementById("agregarPaisProvincia").value
		)
	);
	setTimeout(() => {
		fetchProvinciasSeccion();
		...
		.then(async (ciudadesPorProvincia) => {
			if (ciudadesPorProvincia.length == 0) {
				const response = await fetch(
					"http://localhost:3000/api/provincias/" + idProvincia,
					{
						method: "DELETE",
						mode: "cors",
						cache: "no-cache",
