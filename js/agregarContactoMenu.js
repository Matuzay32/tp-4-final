...
var fetchProvincias = (paisId, provinciaIdSeleccionado) => {
  ...
  provincia.innerHTML = '<option value="" selected="">Seleccione su provincia</option>';
  ciudad.innerHTML = '<option value="" selected="">Seleccione su ciudad</option>';
  ...
  .then(response => response.json())
   .then((data) => {
      ...
      if(provinciaIdSeleccionado) {
        document.getElementById("provinciaMenuNuevoContacto").value = provinciaIdSeleccionado;
      }
   });
}
  ...
  .then(response => response.json())
   .then((data) => {
      ...
      if(ciudadIdSeleccionado) {
        document.getElementById("ciudadMenuNuevoContacto").value = ciudadIdSeleccionado;
      }
...
const response = await fetch("http://localhost:3000/api/contactos", {
  ...
  headers: {
    ...
    'user-token':ObtenerTokenLocalStorage("Contacto agregado con exito","Intente ingresar a la sesion nuevamente para crear un contacto") ,
...
var metodoPostContactoPorImportacion        = async  (data,token) =>{
  const response = await fetch("http://localhost:3000/api/contactos/porImportacion", {
    method: 'POST', 
    mode: 'cors', 
    cache: 'no-cache', 
    credentials: 'same-origin', 
    headers: {
      'Accept': 'application/json',
      'Content-Type': 'application/json',
      'user-token':ObtenerTokenLocalStorage("Contactos agregados con exito","Intente ingresar a la sesion nuevamente para crear contactos") ,
