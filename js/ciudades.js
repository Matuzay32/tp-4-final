...
var metodoPostborrarCiudad    = async (data, token) => {
    const response = await fetch("http://localhost:3000/api/ciudades", {
        ...
        headers: {
            ...
            'user-token': ObtenerTokenLocalStorage("Ciudad borrada con exito","Ingrese nuevamente a la sesion para poder borrar una ciudad"), //Obtengo el token del secion Storage
...
var metodoPostAgregarCiudad   = async (data, token) => {
    const response = await fetch("http://localhost:3000/api/ciudades", {
        ...
        headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json',
            'user-token': ObtenerTokenLocalStorage("Ciudad agregada con exito","Intente ingresar de nuevo para poder agregar una ciudad"), //Obtengo el token del secion Storage
...
btnAgregarCiudad.addEventListener("click", () => {
    ...
    metodoPostAgregarCiudad(crearCiudadBaseDatos(agregarCiudadInput.value, document.getElementById("agregarProvinciaCiudad").value));
    containerListaCiudades.innerHTML = "";
    setTimeout(() => {
        fetchCiudadesSeccion();
        arrayDeObjetos = [];
